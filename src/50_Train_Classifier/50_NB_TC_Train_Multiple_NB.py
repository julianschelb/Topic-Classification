...
def train_model_nb(train_data, german_stop_words, features, max_features=10000):
    """Trains an NB model and returns the model and vectorizer."""
...
for topic in TOPICS:  # ----------------------------------------------------------------------
    ...
    # Train Model
    model, vectorizer = train_model_nb(
    ...
    dataset.save_to_disk(
        f"../../data/tmp/processed_dataset_{topic}_buffed_chunkified_{SAMPLING}{SUFFIX}_{MAX_CONTENT_LENGTH}_s_NB_{FEATURES}_{SPLIT}")
...
file_path = f"eval_results_nb_{FEATURES}_{SPLIT}_chunks.json"
...
# %%
# Define the file path to save the dictionary
file_path = f"eval_results_nb_{FEATURES}_{SPLIT}_pages.json"

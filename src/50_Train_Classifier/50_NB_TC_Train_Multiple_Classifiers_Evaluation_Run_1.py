...
for topic in TOPICS:  # ----------------------------------------------------------------------
    ...
    for model_name in MODELS:  # -------------------------------------------------------------
        ...
        model_name_local = f"../../models_ccu/{model_name.replace('/','_')}_{topic}_model_{FEATURES}_run_{RUN}/"
        ...
        dataset.save_to_disk(
            f"../../data/tmp/processed_dataset_{topic}_buffed_chunkified_{SAMPLING}{SUFFIX}_{MAX_CONTENT_LENGTH}_s_{model_name.split('/')[1]}_{FEATURES}_{SPLIT}_run_{RUN}")
...
file_path = f"eval_results_{FEATURES}_{SPLIT}_chunks_run_{RUN}.json"
...
# %%
# Define the file path to save the dictionary
file_path = f"eval_results_{FEATURES}_{SPLIT}_pages_run_{RUN}.json"
